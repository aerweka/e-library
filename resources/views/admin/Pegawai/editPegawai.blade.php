	...
	<div class="card-body">
		<form action="/pegawai/updateData" method="post" id="editPegawai">
			...
			<div class="form-group">
				...
				<input type="text" name="nip" class="form-control" value="{{ $pegawai[0]->nip }}">
			</div>
			<div class="form-group">
				<label for="inputNama">Nama Pegawai</label>
				<input type="text" name="name" class="form-control" value="{{ $pegawai[0]->nama_pegawai }}">
			</div>
			<div class="form-group">
				<label for="inputUsername">Username</label>
				<input type="text" name="username" class="form-control" value="{{ $pegawai[0]->username }}">
			</div>
			<div class="form-group">
				<label for="exampleInputEmail1">Password</label>
				<input type="password" name="password" class="form-control" value="{{ $pegawai[0]->password }}">
			</div>
			<label for="inputStatus">Status</label>
			<div class="form-check">
				<input class="form-check-input" type="radio" name="status" value="1" {{ $pegawai[0]->status == '1' ? 'checked' : '' }}>
				<label class="form-check-label">Aktif</label>
			</div>
			<div class="form-check">
				<input class="form-check-input" type="radio" name="status" value="0" {{ $pegawai[0]->status == '0' ? 'checked' : '' }}>
